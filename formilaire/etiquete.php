...
?>
<main>
    <form action='includes/enregist.inc.php' method='get'>
    <table class="styled-table" style="margin: 10% 0%;">
    <caption style="background-color: yellow;">CAISSES ENREGISTREUSES</caption>
        <thead>
            <tr>
                <td>id</td>
        ...
        <tbody>
            <?php
                if (isset($_SESSION['enregist'])) {
                    for ($i=0 ; $i < count($_SESSION['enregist']) ; $i++ ) {
                        echo "<tr id='tr".$i."'>";
                        echo "<td ><p id='id".$i."'>".$_SESSION['enregist'][$i]['enrg_id']."</p></td>";
                        echo "<td id='Série".$i."'>".$_SESSION['enregist'][$i]['Série']."</td>";
                        echo "<td id='BARRE".$i."'>".$_SESSION['enregist'][$i]['BARRE']."</td>";
                        echo "<td id='MONTANT".$i."'>".$_SESSION['enregist'][$i]['MONTANT']."</td>";
                        echo "<td id='MODELE".$i."'>".$_SESSION['enregist'][$i]['MODELE']."</td>";
                        echo "<td id='AFFECTATION".$i."'>".$_SESSION['enregist'][$i]['AFFECTATION']."</td>";
                        echo "<td id='PRODUCTION".$i."'>".$_SESSION['enregist'][$i]['PRODUCTION']."</td>";
                        echo "<td id='save".$i."'><input type='button' class='btn' id='".$i."' onClick='modify(this.id)' name='submit-save' value='modify'></td>";
                        echo "<td id='delete".$i."'><input type='text' name='enrg_id' value='".$_SESSION['enregist'][$i]['enrg_id']."' style='display : none;'><button class='btn' type='submit' name='submit-delete'>delete</button></td>";
                        echo "</tr>";
                    }    
                }
...
<script>
    function modify(clicked) {
        ...
        var AFFECTATION = document.getElementById('AFFECTATION'+clicked);
        ...
        if (btn.value != 'save'){
            ...
            AFFECTATION.innerHTML = "<input class='inp' type='text' name='AFFECTATION' value='"+AFFECTATION.textContent+"' id='btn"+clicked+"' ><input type='text' name='cmd_id' value='"+id.textContent+"' style='display : none;'>";
    ...
    function add(clicked) {
        ...
        if (btn.value != 'save'){
            ...
            AFFECTATION.innerHTML = "<input class='inp' type='text' name='AFFECTATION' value='' id='btn"+clicked+"' >";
