...
?>
<main>
    <form action='includes/point.inc.php' method='get'>
    <table class="styled-table" style="margin: 10% 0%;">
    <caption style="background-color: yellow;">HORODATEURS DE POINTAGE</caption>
        <thead>
            <tr>
                <td>id</td>
        ...
        <tbody>
            <?php
                if (isset($_SESSION['point'])) {
                    for ($i=0 ; $i < count($_SESSION['point']) ; $i++ ) {
                        echo "<tr id='tr".$i."'>";
                        echo "<td ><p id='id".$i."'>".$_SESSION['point'][$i]['point_id']."</p></td>";
                        echo "<td id='Série".$i."'>".$_SESSION['point'][$i]['Série']."</td>";
                        echo "<td id='BARRE".$i."'>".$_SESSION['point'][$i]['BARRE']."</td>";
                        echo "<td id='MONTANT".$i."'>".$_SESSION['point'][$i]['MONTANT']."</td>";
                        echo "<td id='MODELE".$i."'>".$_SESSION['point'][$i]['MODELE']."</td>";
                        echo "<td id='IP".$i."'>".$_SESSION['point'][$i]['IP']."</td>";
                        echo "<td id='MAC".$i."'>".$_SESSION['point'][$i]['MAC']."</td>";
                        echo "<td id='EMPLACEMENT".$i."'>".$_SESSION['point'][$i]['EMPLACEMENT']."</td>";
                        echo "<td id='PRODUCTION".$i."'>".$_SESSION['point'][$i]['PRODUCTION']."</td>";
                        echo "<td id='save".$i."'><input type='button' class='btn' id='".$i."' onClick='modify(this.id)' name='submit-save' value='modify'></td>";
                        echo "<td id='delete".$i."'><input type='text' name='point_id' value='".$_SESSION['point'][$i]['point_id']."' style='display : none;'><button class='btn' type='submit' name='submit-delete'>delete</button></td>";
                        echo "</tr>";
                    }    
                }
